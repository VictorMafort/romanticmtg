...
# -*- coding: utf-8 -*-
"""
Romantic Format Tools - v13.10c
Fixes:
- Aba 3: após limpar cache, algumas cartas ficavam "invisíveis" (imagem None).
  Agora **não renderizo** cartas sem imagem (evita espaços vazios) e adiciono *retry* interno no fetch.
- Aba 1: **voltei** a exibir o chip de **Legal/Not Legal/Banned** dentro da carta.
Outros:
- Mantém donuts Altair (Aba 4), DFC image fix, e demais ajustes das versões anteriores.
"""
import re
import time
import urllib.parse
...
def fetch_card_data(card_name, _legal_salt: str = _ALLOWED_FPRINT):
    ...
    - Pequeno *retry* para reduzir falhas momentâneas
    """
...
# --------------------
# Tab 1 — Sugestões (com chip de legalidade de volta)
# --------------------
with tab1:
...
# --------------------
# Tab 2 — Decklist Checker (idêntica à anterior)
# --------------------
with tab2:
    st.write("Cole sua decklist abaixo (uma carta por linha):")
    deck_input = st.text_area("Decklist", height=240)
...
# --------------------
# Tab 3 — Artes (ignora cartas sem imagem para evitar "invisíveis")
# --------------------
with tab3:
    ...
    if not st.session_state.deck:
        ...
        with st.spinner("Carregando artes..."):
            with ThreadPoolExecutor(max_workers=min(6, max(1, len(names)))) as ex:  # limita concorrência
        ...
        for sec in order:
            ...
            for i in range(0, len(group), cols_per_row):
                ...
                for col, (name, qty_init, _t, img, s_text, s_type) in zip(cols, row):
                    ...
                    if qty <= 0:
                        continue
                    if not img:  # evita card invisível
                        skipped += 1
                        continue
                    with col:
                        card_ph = st.empty()
                ...
                st.markdown("---")
        if skipped:
            st.caption(f"ℹ️ {skipped} carta(s) não renderizada(s) por falta de imagem no momento (evitando "
                       f"espaços invisíveis). Isso se normaliza conforme o cache repopula.")

# --------------------
# Tab 4 — (permanece como na 13.10b: donuts Altair e tabela sem índice)
# --------------------
with tab4:
    ...
    if not st.session_state.deck:
        ...
        def donut_altair(df_vals: pd.DataFrame, label_col: str, value_col: str, color_map: dict):
            ...
            return (arc + txt).properties(height=300)
